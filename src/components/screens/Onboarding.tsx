...
export default function Onboarding({ onComplete }: { onComplete: () => void }) {
  ...
  if (phase === 'splash') {
    return (
      ...
      >
        {/* Сліди Каспера — кастомна ілюстрація */}
        <img
          src={`${import.meta.env.BASE_URL}cat-paw-2.png`}
          alt="Сліди Каспера"
          width={180}
          height={120}
    ...
    <div style={{ minHeight: '100dvh', background: '#FFF7EC' }} className="flex flex-col">
      ...
      <div className="px-6 pb-10">
        <Button onClick={next} disabled={saving}>
          {phase === 7 ? (saving ? 'Зберігаємо…' : 'Почати 🐾') : 'Далі →'}
        </Button>
      </div>
    </div>
  )
...
function Step4({ draft, update }: StepProps) {
  ...
  return (
    <>
      ...
      {draft.mode === 'olena' && (
        <div className="rounded-2xl px-4 py-3 mb-4 text-sm" style={{ background: '#CDE1D5', color: '#053E35' }}>
          ⚠️ Режим відновлення. Перед кожним тренуванням Каспер запитає стан зони операції.
        </div>
      )}
